 ...
 */
function authorizePermissions(spreadsheetId) {
  try {
    // スプレッドシート権限
    const sheet = getTargetSpreadsheetResearch(spreadsheetId).getSheetByName(SHEET_NAMES.SETTINGS);
    const testValue = sheet.getRange('A1').getValue();
 ...
 */
function setupOnEditTrigger(spreadsheetId) {
 ...
 */
function fetchCategoryFromUrl(spreadsheetId, url, sheet) {
  const ss = getTargetSpreadsheetResearch(spreadsheetId);
  try {
    // 処理中メッセージを表示
    ss.toast('カテゴリ情報を取得中...', 'eBay API', 3);
    ...
    if (categoryId) {
      setConditionDropdown(spreadsheetId, categoryId.toString(), sheet);
    }

    // 完了メッセージ
    ss.toast('カテゴリ情報を取得しました', '✅ 完了', 2);
 ...
 * @param {Sheet}  sheet リサーチシート
 */
function fetchSpecFromUrl(spreadsheetId, url, sheet) {
  const ss = getTargetSpreadsheetResearch(spreadsheetId);
  try {
    ss.toast('スペック情報を取得中...', 'eBay API', 5);
    ...
    if (existingCategoryId && existingCategoryId !== newCategoryId) {
      ...
      setConditionDropdown(spreadsheetId, newCategoryId, sheet);
 ...
 */
function testOnEdit(spreadsheetId) {
  try {
    const ss = getTargetSpreadsheetResearch(spreadsheetId);
